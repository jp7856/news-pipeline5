# Top News Content Pipeline v5 — 핸드오프 문서

## 프로젝트 개요

영어 교육 신문(Top News Kinder/Kids/Junior/Times/Junior M) 콘텐츠 자동 생성 파이프라인.
...
Generate → 레벨로 에이전트 1-1~1-5 라우팅 (create_agent1, 지침: agents/guidelines/*.md)
         ...
         → [발행하기] → 시트 상태 '발행완료' → /api/published → top-news-weekly 노출
...
10. 발행 → top-news-weekly 노출 (GitHub 토큰 불필요, Railway API 직접 호출)
...
dashboard/app.py           # Flask 앱
                           ...
                           #   /api/published — 발행 기사 목록 (top-news-weekly 전용)
...
- GITHUB_TOKEN — **불필요** (top-news-weekly는 Railway API 직접 호출)
- 점검: GET /api/health
...
- [ ] top-news-weekly 레포에서 Railway URL을 `web-production-0763d.up.railway.app`으로 설정
