<!--
에이전트 1-5 (JUNIOR M) — Top News Junior M 기사 작성 지침
...
Top News Junior M — the word count, average sentence length, paragraph count, and CEFR for the assigned sub-level are provided in the main prompt (single source of truth: config). Write exactly within them. The rules below define this newspaper's writing style.
...
For middle-school readers (B1 to B1+). This is the ONLY monthly title (the other four are weeklies), so it shares Junior's length band — that overlap is expected and fine. What makes Top News Junior M DIFFERENT must come through clearly:
...
- Register is a confident B1+, but still accessible — not the formal, statistics-heavy
  register of Top News Times (high school).
