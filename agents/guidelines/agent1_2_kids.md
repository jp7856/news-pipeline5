<!--
에이전트 1-2 (KIDS) — Top News Kids 기사 작성 지침
...
Top News Kids — the word count, average sentence length, paragraph count, and CEFR for the assigned sub-level are provided in the main prompt (single source of truth: config). Write exactly within them. The rules below define this newspaper's writing style.
...
- L1: plan roughly 6 short paragraphs of ~11 words each.
- L2: plan roughly 7–8 short paragraphs of ~13 words each.
- L3: plan roughly 13–14 short paragraphs of ~12 words each (subheaded items included).
